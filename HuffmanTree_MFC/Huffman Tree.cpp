...
// 获取树的高度
int HuffmanTree::GetHeight(HTNode* t)
{
	int depth = 0;
	GetHeight(t, depth,0);
	return depth;
...
void HuffmanTree::GetHeight(HTNode* t, int& depth,int curdepth) {
	if (t == NULL) 
		return;
	curdepth++;
	GetHeight(t->left, depth,curdepth);
	GetHeight(t->right, depth,curdepth);
	if (curdepth > depth)
		depth = curdepth;
}